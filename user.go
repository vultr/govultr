...
type UserService interface {
	...
	CreateAPIKey(ctx context.Context, userID string, apiKeyReq *APIKeyCreate) (*APIKey, *http.Response, error)
	GetAPIKey(ctx context.Context, userID string, apiKeyID string) (*APIKey, *http.Response, error)
	DeleteAPIKey(ctx context.Context, userID string, apiKeyID string) error
	ListAPIKeys(ctx context.Context, userID string) ([]APIKey, *http.Response, error)
}
...
type UserReq struct {
	...
	// Deprecated: No longer supported as of 07/2025
...
// CreateAPIKey adds an API key to the target user's API key list
func (u *UserServiceHandler) CreateAPIKey(ctx context.Context, userID string, apiKeyReq *APIKeyCreate) (*APIKey, *http.Response, error) {
	uri := fmt.Sprintf("%s/%s/apikeys", path, userID)
	req, err := u.client.NewRequest(ctx, http.MethodPost, uri, apiKeyReq)
	if err != nil {
		return nil, nil, err
	}

	apiKey := new(apiKeyBase)
	resp, err := u.client.DoWithContext(ctx, req, &apiKey)
	if err != nil {
		return nil, resp, err
	}
...
// GetAPIKey gets information about a user's API key
func (u *UserServiceHandler) GetAPIKey(ctx context.Context, userID, apiKeyID string) (*APIKey, *http.Response, error) {
	uri := fmt.Sprintf("%s/%s/apikeys/%s", path, userID, apiKeyID)
	req, err := u.client.NewRequest(ctx, http.MethodGet, uri, nil)
	if err != nil {
		return nil, nil, err
	}
...
// DeleteAPIKey deletes an API key from the target user's API key list
func (u *UserServiceHandler) DeleteAPIKey(ctx context.Context, userID, apiKeyID string) error {
	uri := fmt.Sprintf("%s/%s/apikeys/%s", path, userID, apiKeyID)
	req, err := u.client.NewRequest(ctx, http.MethodDelete, uri, nil)
	if err != nil {
		return err
...
// ListAPIKeys gets all API keys for the target user
func (u *UserServiceHandler) ListAPIKeys(ctx context.Context, userID string) ([]APIKey, *http.Response, error) {
	uri := fmt.Sprintf("%s/%s/apikeys", path, userID)
	req, err := u.client.NewRequest(ctx, http.MethodGet, uri, nil)
	if err != nil {
		return nil, nil, err
	...
	resp, err := u.client.DoWithContext(ctx, req, &apiKeys)
	if err != nil {
		return nil, resp, err
	}
